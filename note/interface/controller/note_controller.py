...
def create_note(
    ...
    # dataclass는 asdict 함수를 이용해 손쉽게 딕셔너리로 변환할 수 있다.
...
def get_notes(
    ...
    total_count, notes = note_service.get_notes(
        user_id=current_user.id, page=page, items_per_page=items_per_page
    )
...
def delete_note(
    id: str,
    current_user: Annotated[CurrentUser, Provide(get_current_user)],
    note_service: NoteService = Depends(Provide[Container.note_service]),
):
